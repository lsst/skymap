...
class SkyTractInfo(object):
    """Information about a sky tract in a SkyMap sky pixelization
    ...
    def __init__(self, id, ctrCoord, vertexCoordList, overlap, wcsFactory):
        ...
        @param[in] overlap: minimum overlap between adjacent sky tracts; an afwGeom.Angle;
            this defines the minimum distance the sky tract extends beyond the inner region in all directions
        ...
        """
#         print "SkyTractInfo(id=%s, ctrCoord=%s, overlap=%0.1f)" % \
#             (id, ctrCoord.getPosition(afwGeom.degrees), overlap)
    ...
    def getCtrCoord(self):
        """Get sky coordinate of center of sky tract (as an afwCoord.Coord)
        """
    ...
    def getOverlap(self):
        """Get overlap with adjacent sky tracts (rad)
        """
        return self._overlap
    
    def getWcs(self):
        """Get WCS of sky tract
    ...
    def __str__(self):
        return "SkyTractInfo(id=%s)" % (self._id,)
    
    def __repr__(self):
        return "SkyTractInfo(id=%s, ctrCoord=%s)" % (self._id, self._ctrCoord.getVector())
